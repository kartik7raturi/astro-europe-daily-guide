...
const SocialShare = ({ title, text, url = window.location.origin, image, hashtags = [] }: SocialShareProps) => {
  ...
  const copyToClipboard = () => {
    ...
    toast({
      ...
      description: "Share this on your social media!"
    });
  };

  const shareOnTwitter = () => {
    const hashtagText = hashtags.length > 0 ? ` ${hashtags.map(tag => `#${tag}`).join(' ')}` : '';
    const tweetText = `${text}${hashtagText}`;
  ...
  const shareOnInstagram = () => {
    ...
    toast({
      ...
      description: "Create a story or post with this text!"
    });
  };
  ...
  const downloadAsImage = () => {
    ...
    if (ctx) {
      ...
      ctx.fillText('astrovibe.online', 400, 550);
    ...
    <Card className="bg-card/80 backdrop-blur-sm border-primary/20">
      ...
      <CardContent>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Button variant="outline" onClick={nativeShare} className="flex items-center gap-2">
          ...
          <Button variant="outline" onClick={shareOnWhatsApp} className="flex items-center gap-2 text-green-600">
          ...
          <Button variant="outline" onClick={shareOnInstagram} className="flex items-center gap-2 text-pink-600">
            <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <Button variant="outline" onClick={downloadAsImage} className="flex items-center gap-2">
